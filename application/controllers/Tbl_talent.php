...
class Tbl_talent extends CI_Controller
{
	...
	public function create_action()
	{
		...
		$data = array(
			...
			'rides' => $this->input->post('rides', TRUE),
			'status' => 'inactive',
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				...
				'rides' => set_value('rides', $row->rides),
	...
	function ubah_profile($id)
	{
		...
		$data = array(
			...
			'rides' => set_value('rides', $row_talent->rides),
			...
			'tiktok' => set_value('tiktok', $row_talent->tiktok),
			'youtube' => set_value('youtube', $row_talent->youtube),
	...
	function send($data)
	{
		...
		// $mail = 'helpdesk@mustika-ratu.co.id';
        ...
        <body>
            <p>Yth. Bapak Johan Jaffarudin,<br/><br/>A request by <b>$row_user->nama_lengkap</b> has been submitted that requires your approval to verified.</p>
            <h4><a href='" . base_url("tbl_talent_verify")."'>Click here to check your approval.</a></h4>
        </body>
        </html>
        ";
	...
	public function send_notif($data)
	{
		//Load data
		$id_users = $data['id_users'];
		$row_user = $this->Tbl_talent_model->get_email_users($id_users);
        $mail = $row_user->username;
		// print_r($mail);
		// die;
        ...
        <html>
        <head>
            <title>Talent Registration Successful</title>
        </head>
        <body>
            <h2>Thank you for Registering Mustika Ratu Talent Account.</h2>
            <p>Your data has uploaded and please wait until your profile is active and ready</p>
		...
		$this->email->from('mustikaratu.mailer@gmail.com', 'Mustika Ratu Talent');
		$this->email->to($mail);
		$this->email->subject('Notification Registration Talent');
		$this->email->message($message);
	...
	public function create_endorse()
	{
		...
		$find = array("<p>","<br>");
		$replace = array("");
		$arr = str_replace("</p>","%0A","$get_endorse->sow");
		$sow = str_replace($find,$replace,$arr);
		// print_r($sow[2]);
		// $data_sow = echo $sow;
		// $sow_endorse = echo $sow;

		redirect('https://api.whatsapp.com/send?phone='.$nomor.'&text=Halo,%20saya%20*'.$get_user->nama_lengkap.'*%0A%0Aingin%20endorse%20talent%20:%20*'.$nama.'*,%0A%0Auntuk%20mempromosian%20:%20'.$get_endorse->endorse.'%0A%0Adengan%20detail%20:%20%0A'.$sow.'%0AJadwal%20endorse%20:%20%0A'.$get_endorse->jadwal.'%0A%0Abudget%20:%20Rp.%20'.$get_endorse->budget.'');
